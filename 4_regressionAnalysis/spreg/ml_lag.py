...
def run_ml_lag(df: pd.DataFrame, df_name, weights, outfile, ax_mi, ax_ms):
    X = df
    ...
    # run spreg.ML_Lag
    mi_model = ML_Lag(y_mi, X_np, name_y="mi_prop", name_x=X_std.columns.tolist(), w=weights, spat_impacts=["simple", "full"])
    ms_model = ML_Lag(y_ms, X_np, name_y="ms_prop", name_x=X_std.columns.tolist(), w=weights, spat_impacts=["simple", "full"])

    feature_names = ["Constant"] + X_filtered.columns.tolist() + ["W_mi_prop"]
...
def main():
    ...
    fig_start.suptitle(
        "ML Lag regression coefficients, trip origin",
        fontsize=16, y=0.99
    )
    axes_start = axes_start.flatten()

    for i, df_name in enumerate(df_start_names):
        run_ml_lag(dfs[df_name], df_name, weights[df_name],
    ...
    fig_end.suptitle(
        "ML Lag regression coefficients, trip destination",
        fontsize=16, y=0.99
    )
